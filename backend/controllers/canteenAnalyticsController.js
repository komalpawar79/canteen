...
export const getCanteenKPIs = async (req, res) => {
  try {
    ...
    const [todayOrders, todayRevenue, activeOrders, totalMenuItems] = await Promise.all([
      Order.countDocuments({ canteen: canteenId, createdAt: { $gte: today }, status: { $ne: 'cancelled' } }),
      Order.aggregate([
        { $match: { canteen: canteen._id, createdAt: { $gte: today }, paymentStatus: 'completed' } },
        { $group: { _id: null, total: { $sum: '$finalAmount' } } }
      ]),
      Order.countDocuments({ canteen: canteenId, status: { $in: ['pending', 'confirmed', 'preparing', 'ready'] } }),
      MenuItem.countDocuments({ canteen: canteenId })
    ]);
