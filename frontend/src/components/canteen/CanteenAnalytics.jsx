...
const CanteenAnalytics = () => {
  ...
  const { kpis, popularItems, loading, fetchKPIs, fetchPopularItems } = useCanteenAnalyticsStore();
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          <div className="text-3xl font-bold text-orange-600">{kpis?.todayOrders || 0}</div>
        </div>
        <div className="bg-white rounded-xl shadow-lg p-6">
          <div className="text-sm text-gray-600 mb-2">Today Revenue</div>
          <div className="text-3xl font-bold text-green-600">Rs.{kpis?.todayRevenue || 0}</div>
        </div>
        <div className="bg-white rounded-xl shadow-lg p-6">
          <div className="text-sm text-gray-600 mb-2">Active Orders</div>
          <div className="text-3xl font-bold text-blue-600">{kpis?.activeOrders || 0}</div>
        </div>
        <div className="bg-white rounded-xl shadow-lg p-6">
          <div className="text-sm text-gray-600 mb-2">Menu Items</div>
          <div className="text-3xl font-bold text-purple-600">{kpis?.totalMenuItems || 0}</div>
        </div>
      </div>
      <div className="bg-white rounded-xl shadow-lg p-6">
